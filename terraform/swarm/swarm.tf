...
resource "openstack_networking_floatingip_v2" "atx2016" {
...
resource "openstack_compute_instance_v2" "atx2016" {
    ...
    floating_ip = "${element(openstack_networking_floatingip_v2.atx2016.*.address, count.index)}"
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "echo 'IP.2 = ${element(openstack_networking_floatingip_v2.atx2016.*.address, count.index)}' >> /tmp/files/ssl/openssl.cnf",
            "echo 'DNS.1 = ${var.fqdn}' >> /tmp/files/ssl/openssl.cnf",
            "echo 'DNS.2 = ${element(openstack_networking_floatingip_v2.atx2016.*.address, count.index)}.xip.io' >> /tmp/files/ssl/openssl.cnf",
